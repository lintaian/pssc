...
;(function ($, window, document, undefined) {
    ...
    function readyDom(iObj) {
        var holder = iObj.holder,
            ...
            outMargin = option.outMargin,
            html = ['<ul class="patt-wrap" style="padding:' + outMargin + 'px">'];
        for (var i = 0, ln = matrix[0] * matrix[1]; i < ln; i++) {
            html.push('<li class="patt-circ" style="margin:' + margin + 'px; width : ' + 
            		(radius * 2) + 'px; height : ' + (radius * 2) + 'px; -webkit-border-radius: ' + 
            		radius + 'px; -moz-border-radius: ' + radius + 'px; border-radius: ' + 
            		radius + 'px; "><div class="patt-dots patt-dots-' + (i+1) + '"></div></li>');
        }
        html.push('</ul>');
        holder.html(html.join('')).css({
            'width': (matrix[1] * (radius * 2 + margin * 2) + outMargin * 2) + 'px',
            'height': (matrix[0] * (radius * 2 + margin * 2) + outMargin * 2) + 'px'
        });
            ...
            if (idx) {
                if (patternAry.indexOf(pattId) == -1) {
                    ...
                    var margin = iObj.option.margin,
                        ...
                        outMargin = iObj.option.outMargin,
                        newX = (posObj.i - 1) * (2 * margin + 2 * radius) +  outMargin + margin + radius,
                        newY = (posObj.j - 1) * (2 * margin + 2 * radius) +  outMargin + margin + radius;
    ...
    PatternLock.prototype = {
        ...
        getPattern: function () {
            return objectHolder[this.token].patternAry && objectHolder[this.token].patternAry.join('');
        },
        reset: function () {
            var iObj = objectHolder[this.token];
            iObj.option.onReset();
